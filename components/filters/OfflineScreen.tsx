...
export function OfflineScreen() {
  ...
  const [direction, setDirection] = useState<"more" | "less">(state.offlineDirection);
  const [value, setValue] = useState(state.offlineValue);

  const hasValue = value.trim().length > 0;

  const apply = () => {
    update({ offlineValue: value.trim(), offlineDirection: direction });
    router.push("/filters");
  };
